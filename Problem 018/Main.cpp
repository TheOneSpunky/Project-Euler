...
static auto maximumPathSum(std::vector<std::vector<int>> triangle) -> int {
  for (auto row { static_cast<long long>(triangle.size() - 2) }; row >= 0; row--)
    for (auto col { long long{0} }; col < static_cast<long long>(triangle[row].size()); col++)
...
auto main() -> int {
  ...
  const auto sum { maximumPathSum(triangle) };
